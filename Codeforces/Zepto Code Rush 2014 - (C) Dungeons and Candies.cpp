...
int main() {
	...
	foru(r, 0, k) {
		...
		edges.pb(mp(n*m, mp(0, r + 1)));
	}
	foru(i, 0, k - 1) {
		foru(j, i + 1, k) {
			int d = dist(i, j);
			edges.pb(mp(d, mp(i + 1, j + 1)));
		}
	}
	...
	while (cnt <= k && i < edges.size()) {
		if (ds.father(edges[i].second.second) != ds.father(edges[i].second.first)) {
			ds.unite(edges[i].second.second, edges[i].second.first);
			res.pb(edges[i]);
			adj[edges[i].second.first].pb(edges[i].second.second);
			adj[edges[i].second.second].pb(edges[i].second.first);
			cnt++;
			s += edges[i].first;
		}
		i++;
