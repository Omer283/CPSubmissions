...
int main() {
	...
	ll ans = 1;
	um<ll, ll> x, primeCount;
	um<ll, ll> mins, secondMins;
	foru(i, 0, n) {
		x = primeDecompose(a[i]);
		for (auto y : x) {
			primeCount[y.first]++;
			if (mins.find(y.first) == mins.end()) mins.insert(y);
			else if (secondMins.find(y.first) == secondMins.end()) secondMins.insert(y);
			else if (mins[y.first] > y.second) {
				secondMins[y.first] = mins[y.first];
				mins[y.first] = y.second;
			}
			else if (secondMins[y.first] > y.second) secondMins[y.first] = y.second;
		}
	}
	for (auto p : primeCount) {
		if (p.second < n - 1) continue;
		if(p.second==n-1)ans *= pw(p.first, mins[p.first]);
		else ans *= pw(p.first, secondMins[p.first]);
	}
	cout << ans << endl;
	return 0;
}