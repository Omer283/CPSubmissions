...
ll fac[(size_t)5e5 + 5], faci[(size_t)5e5 + 5];
...
void precalc(int n) {
	foru(i, 0, n + 1) {
		if (!i) {
			...
			faci[i] = mult(faci[i-1], modpow(i, MOD - 2));
		}
	}
}

ll choose(ll n, ll k) {
	return mult(fac[n], mult(faci[k], faci[n - k]));
}
...
int main() {
	...
	foru(i, 1, n+1) {
		int options = n / i - 1;
		if (options < k - 1)break;
		res = add(res, choose(options, k - 1));
	}
	cout << res << endl;
	return 0;
}